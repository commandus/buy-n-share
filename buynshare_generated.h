...
struct Purchase FLATBUFFERS_FINAL_CLASS : private flatbuffers::Table {
  ...
  const flatbuffers::Vector<uint32_t> *votes() const {
    return GetPointer<const flatbuffers::Vector<uint32_t> *>(VT_VOTES);
  }
  bool Verify(flatbuffers::Verifier &verifier) const {
    return VerifyTableStart(verifier) &&
...
struct PurchaseBuilder {
  ...
  void add_votes(flatbuffers::Offset<flatbuffers::Vector<uint32_t>> votes) {
...
inline flatbuffers::Offset<Purchase> CreatePurchase(
    ...
    uint32_t start = 0,
    uint32_t finish = 0,
    flatbuffers::Offset<flatbuffers::Vector<uint32_t>> votes = 0) {
...
inline flatbuffers::Offset<Purchase> CreatePurchaseDirect(
    ...
    uint32_t start = 0,
    uint32_t finish = 0,
    const std::vector<uint32_t> *votes = nullptr) {
  return bs::CreatePurchase(
      ...
      start,
      finish,
      votes ? _fbb.CreateVector<uint32_t>(*votes) : 0);
}
